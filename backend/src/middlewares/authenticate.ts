...
const local = (req: Request, res: Response, next: NextFunction) => {
  ...
  passport.authenticate(
    "local",
    passportCallback(req, res, next, unauthorizedMessage)
  )(req, res, next);
};

const jwt = (req: Request, res: Response, next: NextFunction) => {
  passport.authenticate("jwt", passportCallback(req, res, next))(
    req,
    res,
    next
  );
...
const jwtRefresh = (req: Request, res: Response, next: NextFunction) => {
  passport.authenticate("jwt-refresh", passportCallback(req, res, next))(
    req,
    res,
    next
  );
...
export const authenticate = authMiddlewares.jwt;
