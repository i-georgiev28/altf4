...
export function PopoverTrigger({ children, className }: PopoverTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "h-9 items-center border border-zinc-950/10 bg-white px-3 text-zinc-950",
        className
      )}
      style={{
        borderRadius: 8,
      ...
      onClick={openPopover}
    >
      <motion.span layoutId={`popover-label-${uniqueId}`} className="text-sm flex flex-row items-center justify-center gap-4">
        {children}
      </motion.span>
    </motion.button>
  )
...
export function PopoverContent({ children, className }: PopoverContentProps) {
    ...
    const formContainerRef = useRef<HTMLDivElement>(null);  // Initialize the ref here
  
    useClickOutside(formContainerRef as RefObject<HTMLElement>, closePopover);
    ...
    return (
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ref={formContainerRef}  // Correct usage of the ref
          layoutId={`popover-${uniqueId}`}
          className={cn(
